    ...
    class lsp{
        ...
        public function logout(){
         ...
         public function selectSum($table,$namaField){
            global $con;
            $sql = "SELECT SUM($namaField) as sum FROM $table WHERE status != '0'";
            $query = mysqli_query($con,$sql);
         ...
         public function selectSumWhere($table,$namaField,$where){
            global $con;
            $sql = "SELECT SUM($namaField) as sum FROM $table WHERE $where AND status != '0'";
            $query = mysqli_query($con,$sql);
            return $data = mysqli_fetch_assoc($query);
        }

        public function selectSumWhereBetween($table,$namaField,$whereparam,$param,$param1){
            global $con;
            $sql = "SELECT SUM($namaField) as sum FROM $table WHERE status != '0' AND $whereparam BETWEEN '$param' AND '$param1'";
            $query = mysqli_query($con,$sql);
            return $data = mysqli_fetch_assoc($query);
        }
        ...
        public function selectBetween($table,$whereparam,$param,$param1){
            global $con;
            $sql = "SELECT * FROM $table WHERE status != '0' AND $whereparam BETWEEN '$param' AND '$param1'";
            $query = mysqli_query($con,$sql);

            $sqls = "SELECT SUM(jumlah) as count FROM $table WHERE status != '0' AND $whereparam BETWEEN '$param' AND '$param1'";
        ...
        public function register($kd_user,$name,$username,$password,$confirm, $foto, $level, $alamat, $no_handphone,$email,$redirect){
            ...
            if($rows == 0){
                ...
                if($password == $confirm){
                    ...
                    $sql = "INSERT INTO table_user VALUES('$kd_user','$name','$username','$password','$response[image]', '$level', '$alamat', '$no_handphone', '$email')";
                    $query   = mysqli_query($con,$sql);
                    if($query){
        ...
        public function selectBarang($table,$where,$whereValues){
            global $con;
            $sql = "SELECT * FROM $table WHERE $where > '$whereValues'";
            $query = mysqli_query($con,$sql);
            $data = [];
